...
boolean  hwSPI;
uint32_t _height=160,_width=120;
//
...
inline uint16_t swapcolor(uint16_t x) { 
 ...
 static inline void spiwrite(uint16_t c)
{
  spi_tx_reg(SPI2_dev, c); // "2. Write the first data item to be transmitted into the SPI_DR register (this clears the TXE flag)."
  while (spi_is_tx_empty(SPI2_dev) == 0); // "5. Wait until TXE=1 ..."
  while (spi_is_busy(SPI2_dev) != 0); // "... and then wait until BSY=0 before disabling the SPI."
}

// Constructor when using hardware SPI.  Faster, but must use SPI pins
// specific to each board type (e.g. 11,13 for Uno, 51,52 for Mega, etc.)
void ST7735(int8_t cs, int8_t rs, int8_t rst) {
  _cs   = cs;
  _rs   = rs;
  _rst  = rst;
  hwSPI = true;
