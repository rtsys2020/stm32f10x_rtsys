...
uint8_t CC1100::begin(SPIClass &spi,uint8_t ss,volatile uint8_t &My_addr)
{
...
//	spi_putc(spi_instr | READ_SINGLE_BYTE);
	_spi->send(spi_instr | READ_SINGLE_BYTE);
	spi_instr = _spi->transfer(0xff);
//	spi_instr = spi_putc(0xff);
	digitalWrite(_ss, HIGH);        // CS high
...
//  while(digitalRead(MOSI_PIN) == 0);   //Wait until MOSI_PIN becomes LOW
//	spi_putc(spi_instr | READ_BURST);
	_spi->send(spi_instr | READ_BURST);


	for(uint8_t i=0; i<length ;i++)
		{
			pArr[i] = _spi->transfer(0xff);
		}
...
//	spi_putc(spi_instr | WRITE_SINGLE_BYTE);
	_spi->send(spi_instr | WRITE_SINGLE_BYTE);
	_spi->send(value);
//  spi_putc(value);
  digitalWrite(_ss, HIGH);
}
...
//	spi_putc(spi_instr | WRITE_BURST);
	_spi->send(spi_instr | WRITE_BURST);
