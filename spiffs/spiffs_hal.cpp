...
//  SerialFlash.eraseSector(sector);
//    return 0;
//}
//
//bool flashWrite(uint32_t offset, uint32_t *data, size_t size) {
//  SerialFlash.write(offset,data,size);
//    return 0;
//}
//
//bool flashRead(uint32_t offset, uint32_t *data, size_t size) {
//  SerialFlash.read(offset,data,size);
//    return 0;
//}
...
//    optimistic_yield(10000);
    ...
    if (addr < alignedBegin) {
        ...
        uint32_t tmp;
        if (!SerialFlash.read(alignedBegin - 4, &tmp, 4)) {
    ...
    if (alignedEnd != alignedBegin) {
        if (!SerialFlash.read(alignedBegin, (uint32_t*) (dst + alignedBegin - addr),
                alignedEnd - alignedBegin)) {
            DEBUGV("_spif_read(%d) addr=%x size=%x ab=%x ae=%x\r\n",
                __LINE__, addr, size, alignedBegin, alignedEnd);
            return SPIFFS_ERR_INTERNAL;
    ...
    if (addr + size > alignedEnd) {
        ...
        uint32_t tmp;
        if (!SerialFlash.read(alignedEnd, &tmp, 4)) {
            DEBUGV("_spif_read(%d) addr=%x size=%x ab=%x ae=%x\r\n",
                __LINE__, addr, size, alignedBegin, alignedEnd);
            return SPIFFS_ERR_INTERNAL;
        }
...
//    optimistic_yield(10000);
    ...
    if (addr < alignedBegin) {
        ...
        if (!SerialFlash.write(alignedBegin - 4, (uint32_t*) tmp, 4)) {
    ...
    if (alignedEnd != alignedBegin) {
        ...
        if (!srcAlign) {
            if (!SerialFlash.write(alignedBegin, (uint32_t*) srcLeftover,
                    alignedEnd - alignedBegin)) {
                DEBUGV("_spif_write(%d) addr=%x size=%x ab=%x ae=%x\r\n",
                    __LINE__, addr, size, alignedBegin, alignedEnd);
                return SPIFFS_ERR_INTERNAL;
            ...
            for (uint32_t sizeLeft = alignedEnd - alignedBegin; sizeLeft; ) {
                ...
                if (!SerialFlash.write(alignedBegin, (uint32_t*) buf, willCopy)) {
                    DEBUGV("_spif_write(%d) addr=%x size=%x ab=%x ae=%x\r\n",
                        __LINE__, addr, size, alignedBegin, alignedEnd);
                    return SPIFFS_ERR_INTERNAL;
                }
    ...
    if (addr + size > alignedEnd) {
        ...
        if (!SerialFlash.write(alignedEnd, &tmp, 4)) {
            DEBUGV("_spif_write(%d) addr=%x size=%x ab=%x ae=%x\r\n",
                __LINE__, addr, size, alignedBegin, alignedEnd);
            return SPIFFS_ERR_INTERNAL;
        }
...
//        optimistic_yield(10000);
        if (!SerialFlash.eraseSector(sector + i)) {
